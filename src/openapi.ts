...
const schemas = {
  ...
  Property: obj({ address: str, askingPrice: num, bedrooms: int, bathrooms: num, parking: int, floorAreaM2: num, erfM2: num, ratesMonthly: num, levyMonthly: num, propertyType: str, titleType: en('freehold', 'sectional', 'unknown'), suburb: str, city: str, title: str, features: arr(str) }, ['askingPrice']),
  ...
  DecisionV1: obj({
    ...
    neighbourhood: obj({
      ...
      location: orNull(obj({ lat: num, lng: num, source: en('listing', 'supplied', 'geocoded'), precision: en('supplied', 'address', 'street', 'suburb'), matchedAs: nstr })),
      geocoding: obj({ status: en('not_needed', 'ok', 'not_found', 'mismatch', 'unavailable', 'disabled'), precision: { type: ['string', 'null'], enum: ['address', 'street', 'suburb', null] }, matchedAs: nstr, attempts: int }),
      ...
      principle: str, mapStatus: en('not_requested', 'disabled', 'no_coordinates', 'unavailable', 'ok'), place: obj({ suburb: nstr, city: nstr, province: nstr }),
      areas: arr(obj({ key: str, label: str, status: en('evidence_found', 'no_evidence'), reason: en('evidence', 'none_found', 'not_retrieved', 'not_requested'), summary: str, items: arr(obj({ name: str, distanceM: t(['integer', 'null']), source: en('listing', 'openstreetmap', 'supplied') })), source: en('listing', 'openstreetmap', 'supplied', 'none'), score: t(['integer', 'null']) })),
    }),
    listing: orNull(obj({
      ...
      capture: obj({ mode: en('browser', 'supplied_html'), sections: arr(obj({ key: str, status: en('captured', 'not_found'), count: t(['integer', 'null']) })), warning: nstr }),
