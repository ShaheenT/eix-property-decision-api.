...
export function coverage(L: Listing) {
  ...
  return { js: { pois: !!L.pois?.length, calculator: !!L.portalCalc, recentSales: !!L.recentSales?.length }, found, missing: FIELDS.filter(f => !has(f)), core: (L.listing === 'rental' ? L.rent !== undefined : has('price')) && has('beds') && has('city') };
}
...
if (/smoke\.(ts|js)$/.test(process.argv[1] ?? '')) {
  ...
  for (const url of file ? readFileSync(file, 'utf8').split('\n').map(l => l.trim()).filter(l => l && !l.startsWith('#')) : []) {
    ...
    try {
      ...
      const p24 = /property24\.com/.test(url), lazy = !c.js.pois && !c.js.calculator && !c.js.recentSales;
      if (!c.core || (p24 && lazy)) failures++;
      if (p24 && lazy) console.log('     WARNING: none of the JavaScript-loaded sections (points of interest, bond calculator, recent sales) were captured');
      console.log(`${c.core && !(p24 && lazy) ? 'OK  ' : 'FAIL'} ${Date.now() - t0}ms ${url}\n     found: ${c.found.join(', ')}${c.missing.length ? `\n     missing: ${c.missing.join(', ')}` : ''}`);
