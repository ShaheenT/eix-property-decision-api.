...
export interface Parsed {
  property?: { askingPrice: number; bedrooms?: number; bathrooms?: number; parking?: number; floorAreaM2?: number; erfM2?: number; ratesMonthly?: number; levyMonthly?: number; propertyType?: string; titleType?: 'freehold' | 'sectional' | 'unknown'; suburb?: string; city?: string; title?: string; address?: string; features: string[] };
  ...
  neighbourhood?: { geocode?: boolean; location?: { lat: number; lng: number }; priorities?: string | Record<string, number>; metrics: Metrics; osm?: boolean };
...
export function parseRequest(b: any): { value?: Parsed; errors?: FieldError[] } {
  ...
  if (b.property !== undefined) {
    ...
    only(P, ['askingPrice', 'bedrooms', 'bathrooms', 'parking', 'floorAreaM2', 'erfM2', 'ratesMonthly', 'levyMonthly', 'propertyType', 'titleType', 'suburb', 'city', 'title', 'address', 'features'], 'property');
    ...
    out.property = { askingPrice: num(P, 'askingPrice', 50_000, 200_000_000, 'property', true)!, bedrooms: num(P, 'bedrooms', 0, 50, 'property', false, true), bathrooms: num(P, 'bathrooms', 0, 50, 'property'), parking: num(P, 'parking', 0, 100, 'property', false, true), floorAreaM2: num(P, 'floorAreaM2', 1, 100_000, 'property'), erfM2: num(P, 'erfM2', 1, 10_000_000, 'property'), ratesMonthly: num(P, 'ratesMonthly', 0, 1_000_000, 'property'), levyMonthly: num(P, 'levyMonthly', 0, 1_000_000, 'property'), propertyType: str(P, 'propertyType', 40, 'property'), titleType: t, suburb: str(P, 'suburb', 80, 'property'), city: str(P, 'city', 80, 'property'), title: str(P, 'title', 200, 'property'), address: str(P, 'address', 200, 'property'), features: Array.isArray(P.features) ? P.features : [] };
  ...
  if (b.neighbourhood !== undefined) {
    ...
    only(N, ['location', 'priorities', 'metrics', 'osm', 'geocode'], 'neighbourhood');
    ...
    if (N.geocode !== undefined) { if (typeof N.geocode === 'boolean') nb.geocode = N.geocode; else bad('neighbourhood.geocode', 'must be a boolean'); }
