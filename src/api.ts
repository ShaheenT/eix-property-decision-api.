...
import { nominatimGeocoder, type Geocoder } from './geocode.js';
...
export interface AppConfig { keys: string; rateLimitPerMin?: number; corsOrigins?: string[]; maxRenders?: number; amenities?: Provider; market?: MarketProvider; geocoder?: Geocoder }
...
async function compute(v: Parsed, amenities: Provider, market?: MarketProvider, geocoder?: Geocoder): Promise<{ d: Decision; rc?: RentCtx; nb: Nb; md: MarketMeta }> {
  ...
  if (v.neighbourhood) {
    ...
    let loc = N.location ?? listing, src: 'listing' | 'supplied' | 'geocoded' = N.location ? 'supplied' : 'listing', precision: 'supplied' | 'address' | 'street' | 'suburb' = 'supplied', matchedAs: string | undefined;
    let geo: Nb['geocoding'] = { status: 'not_needed', precision: null, matchedAs: null, attempts: 0 };
    if (!loc && N.osm !== false) {
      if (N.geocode === false || !geocoder) geo = { ...geo, status: 'disabled' };
      else if (!d.L.streetAddress && !d.L.suburb) geo = { ...geo, status: 'not_found' };
      else {
        const g = await geocoder({ address: d.L.streetAddress, suburb: d.L.suburb, city: d.L.city, province: d.L.province });
        geo = { status: g.status, precision: g.result?.precision ?? null, matchedAs: g.result?.matchedAs ?? null, attempts: g.attempts };
        if (g.result) { loc = { lat: g.result.lat, lng: g.result.lng }; src = 'geocoded'; precision = g.result.precision; matchedAs = g.result.matchedAs; }
      }
    }
    nb = await assessNeighbourhood({ location: loc, locationSource: src, precision, matchedAs, priorities: N.priorities, metrics: N.metrics, useOsm: N.osm }, amenities);
    nb.geocoding = geo;
...
async function base(v: Parsed, comps: Parsed['comparables']) {
  ...
  if (v.property) {
    const P = v.property;
    const L: Listing = { url: 'supplied', title: P.title, suburb: P.suburb, streetAddress: P.address, city: P.city, province: provinceOf(P.city), propertyType: P.propertyType, titleType: P.titleType ?? 'unknown', price: P.askingPrice, beds: P.bedrooms, baths: P.bathrooms, parking: P.parking, floorSqm: P.floorAreaM2, erfSqm: P.erfM2, levy: P.levyMonthly, rates: P.ratesMonthly, features: P.features.map(f => f.toLowerCase()), src: {} };
...
export function createApp(cfg: AppConfig) {
  const geocoder = cfg.geocoder ?? (process.env.GEOCODER === 'off' ? undefined : nominatimGeocoder());
  ...
  const server = http.createServer(async (req, res) => {
    ...
    try {
      ...
      try { out = await compute(v, cfg.amenities ?? osmProvider, cfg.market, geocoder); }
      catch (e) {
