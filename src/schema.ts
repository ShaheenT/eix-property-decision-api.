...
import { NOT_REQUESTED, publicNeighbourhood, type Nb } from './neighbourhood.js';
...
export function toPublic(d: Decision, id: string, rc?: RentCtx, nb: Nb = NOT_REQUESTED, md: MarketMeta = { source: 'none', status: 'ok', salesReceived: 0, rentalsReceived: 0 }) {
  ...
  return {
    ...
    neighbourhood: publicNeighbourhood(nb, d),
...
/** Says exactly which page sections were captured, so a gap is reported as "not loaded", never as "nothing there". */
function captureReport(L: Decision['L']) {
  const secs = [{ key: 'listingDetails', n: L.listingNumber && L.listedDate ? 1 : 0 }, { key: 'pointsOfInterest', n: L.pois?.length ?? 0 }, { key: 'bondCalculator', n: L.portalCalc ? 1 : 0 }, { key: 'recentSales', n: L.recentSales?.length ?? 0 }];
  const lazyMissing = secs.slice(1).every(s => !s.n), mode = L.captureMode ?? 'supplied_html';
  return {
    mode, sections: secs.map(s => ({ key: s.key, status: s.n ? 'captured' : 'not_found', count: s.n || null })),
    warning: !lazyMissing ? null : mode === 'browser' ? 'The page was rendered but the points of interest, bond calculator and recent sales did not load. Retry, or run npm run smoke to check the renderer.' : 'Points of interest, the bond calculator and recent sales are added by JavaScript after the page loads. Send the url so the API renders it in a browser, or supply HTML captured after the page finished loading.',
  };
}
