...
BeforeAll(async () => {
  ...
  server = createApp({ keys: 'test:sk_test,limited:sk_limited:2', rateLimitPerMin: 1000, market: httpMarketProvider(mockUrl, 'mk'), geocoder: async q => {
    if (q.address?.includes('Broken')) return { status: 'unavailable', attempts: 1 };
    if (q.address?.includes('Nowhere')) return { status: 'not_found', attempts: 3 };
    return { status: 'ok', attempts: q.address ? 1 : 3, result: { lat: -33.93, lng: 18.47, precision: q.address ? 'address' : 'suburb', matchedAs: `${q.address ?? q.suburb}, ${q.city}` } };
  }, amenities: async lat => { if (lat === -33.94) throw new Error('down'); if (lat === -33.95) return []; return FIXTURE; } });
...
When('I analyse the saved live listing {string} requesting the neighbourhood layer with key {string}', async function (this: A, f: string, key: string) {
  await call(this, 'POST', '/v1/decisions', key, JSON.stringify({ url: 'https://www.property24.com/for-sale/woodstock/cape-town/western-cape/10164/117106348', html: readFileSync(`features/fixtures/${f}`, 'utf8'), neighbourhood: {} }));
});

When('I analyse the saved live listing {string} requesting the neighbourhood layer without geocoding with key {string}', async function (this: A, f: string, key: string) {
  await call(this, 'POST', '/v1/decisions', key, JSON.stringify({ url: 'https://www.property24.com/for-sale/woodstock/cape-town/western-cape/10164/117106348', html: readFileSync(`features/fixtures/${f}`, 'utf8'), neighbourhood: { geocode: false } }));
});
